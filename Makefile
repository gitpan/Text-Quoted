# This Makefile is for the Text::Quoted extension to perl.
#
...
#     ABSTRACT_FROM => q[Quoted.pm]
#     AUTHOR => q[Jesse Vincent <jesse@bestpractical.com>]
#     DIR => []
#     DISTNAME => q[Text-Quoted]
#     NAME => q[Text::Quoted]
#     NO_META => q[1]
...
TO_INST_PM = Quoted.pm

PM_TO_BLIB = Quoted.pm \
	$(INST_LIB)/Text/Quoted.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  Quoted.pm $(INST_MAN3DIR)/Text::Quoted.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)/Text/Quoted.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Text-Autoformat" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Text-Tabs" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  Quoted.pm $(INST_LIB)/Text/Quoted.pm 
	$(NOECHO) $(TOUCH) $@
